...
public class PacketLogger extends Module {
    public PacketLogger() {
        super(Pixel.CATEGORY, "Logger", "Logs packets or whatever you want. (only packets rn)");
    }
...
// TODO       if (packet instanceof CustomPayloadC2SPacket p) return "CustomPayload data:" + p.getData() + " channel:" + p.payload().id();
    ...
    private String recipes(List<RecipeEntry<?>> list) {
        ...
        for (RecipeEntry<?> r : list) {
            builder.append(r.value().createIcon().getItem().getName()).append("\n");
        }
        return builder.toString();
    }
